...
const SettingComp = () => {
  ...
  const [currentSecondNavBarItem, setCurrentSecondNavBarItem] = useState(0)
  ...
  const changeCurrentSecNavItemIndex = (index) => {
    return () => {
      setCurrentSecondNavBarItem(index)
    }
  }
    ...
    <div className="settings-main">
      <div className="second-nav-bar">
        {secNavBarList.map((item, index) => (
          // if the item is the current page, add the class 'active'

          <div
            onClick={changeCurrentSecNavItemIndex(index)}
            key={index}
            className={
              index == currentSecondNavBarItem
                ? 'second-nav-bar-item active'
                : 'second-nav-bar-item'
            }
          >
            {item}
        ...
        <div className="maincontent">
          ...
          <div className="header">
            <div className="avatar">
              ...
              {petOwner.firstName[0].toUpperCase() +
                petOwner.lastName[0].toUpperCase()}
            </div>
            <div className="head-info">
              ...
              {petOwner.pets && petOwner.pets.length > 0 && (
                <div className="pet-owner-name">
                  <div className="pet-owner-name-item1">
                    {petOwner.pets[0].type}
                  </div>
                  :
                  <div className="pet-owner-name-item2">
                    {petOwner.pets[0].name}
                  </div>
                </div>
              )}
            </div>
      ...
      {currentSecondNavBarItem == 1 && (
        <div className="maincontent">Notification</div>
      )}

      {currentSecondNavBarItem == 2 && (
        <div className="maincontent">Payment & Credits</div>
      )}

      {currentSecondNavBarItem == 3 && (
        <div className="maincontent">Password & Security</div>
      )}
    </div>
  )
}
